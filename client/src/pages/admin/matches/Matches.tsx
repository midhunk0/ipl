...
import type { MatchInputType, PlayoffsInputType } from "../../../types/type";
...
const initialPlayOffData: PlayoffsInputType={
    qualifier1: {
        date: "",
        time: "19:30",
        venue: "",
...
export function Matches(){
    ...
    const [playoffData, setPlayoffData]=useState<PlayoffsInputType>(initialPlayOffData)
    ...
    const [showOptions3, setShowOptions3]=useState<boolean>(false);
    const [showOptions4, setShowOptions4]=useState<boolean>(false);
    ...
    function handleTeamSelection(team: string) {
        setMatchData(prev=>({
            ...prev,
            teamShort: team, 
        }));
    ...
    function handleVenueSelection(venue: string) {
        setMatchData(prev=>({
            ...prev,
            venue: venue, 
        }));
    ...
    async function handleAddMatches(e: React.FormEvent<HTMLFormElement>){
        e.preventDefault();
        if(!fileInput){
    ...
    async function handleSetPlayoff(e: React.FormEvent<HTMLFormElement>){
        e.preventDefault();
        try{
        ...
        <div className="admin-matches">
            ...
            {viewMode==="addAll" && 
                <form className="admin-matches-upload-form" onSubmit={handleAddMatches} method="POST">
                    <input type="file" name="fileInput" className="input" onChange={(e: React.ChangeEvent<HTMLInputElement>)=>{if (e.target.files && e.target.files.length > 0) {
                                    ...
                                    <div className="select">
                                        {season.teams.map((team)=>(
                                            team.home.map((home, index)=>(
                                                <div className="input" key={index} onClick={(e: React.MouseEvent<HTMLDivElement>)=>{ e.stopPropagation(); setPlayoffData({...playoffData, qualifier1: { ...playoffData.qualifier1, venue: home }}); setShowOptions1(false)}}>
                                                    <p>{home}</p>
                                                </div>
                                            ))
                                        ))}
                            ...
                            <div className="input selection" onClick={()=>setShowOptions2(prev=>!prev)}>
                                ...
                                {showOptions2 && 
                                    <div className="select">
                                        {season.teams.map((team)=>(
                                            team.home.map((home, index)=>(
                                                <div className="input" key={index} onClick={(e)=>{ e.stopPropagation(); setPlayoffData({ ...playoffData, eliminator: { ...playoffData.eliminator, venue: home }}); setShowOptions2(false)}}>
                                                    <p>{home}</p>
                                                </div>
                                            ))
                                        ))}
                            ...
                            <div className="input selection" onClick={()=>setShowOptions3(prev=>!prev)}>
                                ...
                                {showOptions3 && 
                                    <div className="select">
                                        {season.teams.map((team)=>(
                                            team.home.map((home, index)=>(
                                                <div className="input" key={index} onClick={(e)=>{ e.stopPropagation(); setPlayoffData({ ...playoffData, qualifier2: { ...playoffData.qualifier2, venue: home }}); setShowOptions3(false)}}>
                                                    <p>{home}</p>
                                                </div>
                                            ))
                                        ))}
                            ...
                            <div className="input selection" onClick={()=>setShowOptions4(prev=>!prev)}>
                                ...
                                {showOptions4 && 
                                    <div className="select">
                                        {season.teams.map((team)=>(
                                            team.home.map((home, index)=>(
                                                <div className="input" key={index} onClick={(e)=>{ e.stopPropagation(); setPlayoffData({ ...playoffData, final: { ...playoffData.final, venue: home }}); setShowOptions4(false)}}>
                                                    <p>{home}</p>
                                                </div>
                                            ))
                                        ))}
                        ...
                        <div className="input selection" onClick={()=>setShowOptions(prev=>!prev)}>
                            ...
                            {showOptions && 
                                <div className="select" onClick={()=>setShowOptions(prev=>!prev)}>
                                    {season.teams.map((team)=>(
                                        <div className="input" key={team._id} onClick={()=>handleTeamSelection(team.short)}>
                                            <p>{team.short}</p>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {showOptions1 && 
                                <div className="select" onClick={()=>setShowOptions1(prev=>!prev)}>
                                    {season.teams.filter(opponent=>opponent.short!==matchData.teamShort)
                                        .map((opponent)=>(
                                            <div className="input" key={opponent._id} onClick={()=>handleOpponentSelection(opponent.short)}>
                                                <p>{opponent.short}</p>
                                            </div>
                                        ))
                                    }
                        ...
                        <div className="input selection" onClick={()=>setShowOptions2(prev=>!prev)}>
                            ...
                            {showOptions2 && 
                                <div className="select" onClick={()=>setShowOptions2(prev=>!prev)}>
                                    {season.teams.map((team)=>(
                                        team.home.map((home, index)=>(
                                            <div className="input" key={index} onClick={()=>handleVenueSelection(home)}>
                                                <p>{home}</p>
                                            </div>
                                        ))
                                    ))}
